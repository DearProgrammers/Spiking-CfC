...
parser.add_argument('--file_name', metavar="file_name", type=str, default='eval1', help='Enter the model name you want to save as')
...
class Net(nn.Module):
    def __init__(self, dr, spike_grad, threshold, beta, kernel_size):
        ...
        self.lstm1 = snn.SConv2dLSTM(in_channels=12, out_channels=16, kernel_size=kernel_size, max_pool=2, threshold=threshold, spike_grad=spike_grad_lstm)
    ...
    def forward(self, x):
        ...
        for step in range(x.size(0)):
            ...
            print(f"Layer 1 success: {spk1.size(), syn1.size(), mem1.size(), type(spk1)}")
            cur2 = self.dropout1(self.fc1(spk1.flatten(1)))
            print(f"Layer 2 success: {cur2.size()}")
            spk2, mem2 = self.lif1(cur2, mem2)
            print(f"Layer 3 success: {spk2.size()}")
            cur3, _ = ncp(spk2)
            print(f"Layer 4 success: {cur3.size()}")
            spk3, mem3 = self.lif2(cur3, mem3)
            print(f"Layer 5 success: {spk3.size()}")
...
metrics_df.to_csv('/results/'+ file_name +'/results.csv', index=False)